...
import com.cdac.billingsoftware.entity.OrderEntity;
import com.cdac.billingsoftware.entity.OrderItemEntity;
import com.cdac.billingsoftware.io.*;
...
public class OrderServiceImpl implements OrderService {
	...
	public List<OrderResponse> getLatestOrders() {
		return orderEntityRepository.findAllByOrderByCreatedAtDesc().stream().map(this::convertToResponse)
				.collect(Collectors.toList());
	}
	...
	public List<OrderResponse> findRecentOrders() {
		return orderEntityRepository.findRecentOrders(PageRequest.of(0, 5)).stream()
				.map(orderEntity -> convertToResponse(orderEntity)).collect(Collectors.toList());
	}
